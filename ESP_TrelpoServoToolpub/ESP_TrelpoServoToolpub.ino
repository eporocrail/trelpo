/*
  ...
  Address of decoder is read from 6 dipswitches.
...
static byte const switchNr = 6;
static byte const dip[switchNr] = {D1, D2, D3, D4, D5, D6};
static byte const Led = D0;
static byte const ack1 = D8;
static byte const ack2 = D7;
...
static boolean ackStraight = false;
static boolean ackThrown = false;
static boolean ackStraightOld = false;
static boolean ackThrownOld = false;
...
static unsigned long straightTimer = millis();
static unsigned long thrownTimer = millis();
static unsigned long nextAddressTimer = millis();

static const byte readPotDelay = 100;
static const byte addressSelectDelay = 100;
static const byte straightDelay = 250;
static const byte thrownDelay = 250;
...
void SelectAddress() {
  ...
  if (millis() - addressSelectTimer >= addressSelectDelay) {
    ...
    if (addressSelected == false) {
      ...
      addressSelected = digitalRead(ack1);                        // read button
...
/*
   ...
   function : read button to confirm servo position straight, publish messsage
*/
void ConfirmStraight() {
  if (millis() - straightTimer >= straightDelay) {
    straightTimer = millis();
    ackStraight = digitalRead(ack1);                            // read button
    if ((ackStraight == HIGH) && (ackThrown == HIGH)) {         // publish message if other button is not pushed
      msgOut[2] = 1;
      flashCounter = 0;
      control = 9;                                              // divert to flash
...
} // end of ConfirmStraight

void ConfirmThrown() {
  if (millis() - thrownTimer >= thrownDelay) {
    thrownTimer = millis();
    ackThrown = digitalRead(ack2);                              // read button
    if ((ackThrown == LOW) && (ackStraight == LOW)) {           // publish message if other button is not pushed
      msgOut[3] = 1;
      flashCounter = 0;
      control = 9;                                              // divert to flash
      stepBack = 0;                                             // restore control loop
